...
static char* xml_strtok_r(char *str, const char *delim, char **nextp) {
	...
	// moves to after any beginning delimiters
	...
	// move str to the next delimiter
	str += strcspn(str, delim);

	// str should be pointing to delimiter right now and we 0 that out
	if (*str) {
		*str++ = '\0';
	}
 ...
 */
struct xml_parser {
	// buffer of xml file data
	uint8_t* buffer;
	// pointer into buffer 
	size_t position;
	// length in bytes of xml data
	size_t length;
};
...
char* xml_get_zero_terminated_buffer(struct xml_string* string) {
	char* buffer = malloc(sizeof(uint8_t) * string->length + 1);
...
static struct xml_node* xml_parse_node(struct xml_parser* parser) {
	...
	// length of the name of the opening tag
